...
# Перевірка GPU (CUDA) для максимального FPS
...
entry_times = {}
entry_times_reverse = {}
current_speeds = {}  # Словник для зберігання швидкості кожного ID
processed_ids = set()
total_cars_count = 0
...
while True:
    ...
    # На потужному ПК можна зменшити кількість пропущених кадрів для більшої плавності
    for _ in range(2):
        cap.grab()
        ...
        for box, track_id in zip(boxes, ids):
            ...
            if is_below_line(pos, LINE_FINISH) and track_id in entry_times:
                ...
                if dt > 0.3:
                    ...
                    current_speeds[track_id] = speed_kmh  # Зберігаємо швидкість для відображення
                    processed_ids.add(track_id)
                    total_cars_count += 1
                    with open(CSV_PATH, 'a', newline='') as f:
                        csv.writer(f).writerow([track_id, time.strftime("%H:%M:%S"), speed_kmh, 'Forward'])

            # --- ЛОГІКА ШВИДКОСТІ (Назад) ---
            ...
            # --- ВІЗУАЛІЗАЦІЯ БІЛЯ АВТО ---
            cv2.rectangle(frame, (x1, y1), (x2, y2), (0, 255, 0), 2)

            # Текст: ID та швидкість (якщо вона вже обчислена)
            display_text = f"ID:{track_id}"
            if track_id in current_speeds:
                display_text += f" | {current_speeds[track_id]} km/h"

            # Малюємо фон під текст для кращої видимості
